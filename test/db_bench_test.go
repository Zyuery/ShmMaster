...
func mustOpenBenchDB(b *testing.B, path string, size int64) *shm_master.DB {
	b.Helper()
	db, err := shm_master.Open(path, size)
	if err != nil {
		b.Fatalf("Open: %v", err)
	}
	return db
...
func warmupKeys(b *testing.B, db *shm_master.DB, n int) {
	b.Helper()
	for i := 0; i < n; i++ {
